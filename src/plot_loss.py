...
def main(args):
    ...
    train_loss = loss_df.groupby('epoch').apply(lambda x: x.iloc[:-1]).reset_index(drop=True)
    train_loss = train_loss.groupby('epoch').agg('mean').reset_index(drop=True).drop(['step'], axis=1)
    valid_loss = loss_df.groupby('epoch').tail(1).reset_index(drop=True).drop(['epoch', 'step'], axis=1)
